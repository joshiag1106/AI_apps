 ...
 *
 * Six chapters need a specific case (2, 4, 5, 6, 8 and the alert scene of 11), so they can fall back
 ...
 *
 * The Lens (3) needs a specific case too, but has no capture: a framing gap is a claim about today's
 * reporting, and only lib/lens/compare's own test may make it. With no gap that passed, it is left out.
 */
export function buildDemoScript(input: DemoInput, fallbacks: Fallbacks = FALLBACKS): DemoScript {
  ...
  const add = <K extends ChapterId>(
    id: K, data: ChapterData[K] | null, source: Source, over: { caption?: string; seconds?: number } = {},
  ) => {
    if (!data) return;
    const m = meta(id);
    const chapter: Chapter<K> = {
      id, title: m.title, caption: over.caption ?? m.caption, seconds: over.seconds ?? m.seconds, source,
  ...
  for (const { id } of CHAPTERS) {
    switch (id) {
      ...
      case 'lens': add('lens', selectLens(input), live); break;
      ...
      case 'network': {
        const n = selectNetwork(input);
        add('network', n, live, n && !n.person ? WALK_ONLY : {});
        break;
      }
      ...
      case 'close': { const c = closingFor(input.claims); add('close', c, live, { caption: c.copy }); break; }
    }
  }
