...
export function LadderGauge({ rung, compact = false }: { rung: number; compact?: boolean }) {
  ...
  return (
    ...
    <div className="panel p-4">
      <div className="text-[12px] uppercase tracking-[0.16em] text-faint">PRC official escalation ladder</div>
      {hit && (
        <div className="mt-2 flex items-baseline gap-2.5">
          <span className="text-xl">
            <ChineseText text={hit.zh} accent clamp={false} />
          </span>
          <span className="text-[15px] text-text">{hit.en}</span>
          <span className="mono-num ml-auto text-[13px] text-faint">rung {hit.rung}/13</span>
        </div>
      )}
      {hit && <p className="mt-1.5 text-[14px] leading-snug text-muted">{hit.gloss}</p>}
          ...
          return (
            <div key={r.rung} className="flex items-center gap-2">
              ...
              <div className="relative h-4 flex-1 overflow-hidden rounded-sm bg-[color:var(--color-line-soft)]">
                ...
                <span className={`absolute inset-y-0 left-1.5 flex items-center text-[12px] ${active ? 'text-[#0a0d13] font-semibold' : 'text-muted'}`}>
