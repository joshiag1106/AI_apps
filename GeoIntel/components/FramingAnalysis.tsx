...
export function BlockedNotice({ why, detail, eventId }: { why: Blocked['why']; detail?: string; eventId: string }) {
  return (
    <div className="mt-3 rounded-md border border-[color:var(--color-line)] px-3 py-2.5 text-[14px] leading-relaxed text-muted">
      {why === 'signin' && (
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <div className="mb-1.5 text-[12px] uppercase tracking-[0.16em] text-faint">{title}</div>
      {children}
    </div>
  );
}
  ...
  return (
    <div className="panel p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="text-[12px] uppercase tracking-[0.16em] text-faint">Optional layer</div>
          <h3 className="mt-0.5 text-[17px] font-semibold tracking-tight">Cross-language framing comparison</h3>
        </div>
        {state.k === 'idle' && enabled && (signedIn ? (
          <button onClick={run}
            className="rounded-md bg-[color:var(--color-accent)] px-3 py-1.5 text-[15px] font-medium text-[#0a0d13] hover:opacity-90">
        ...
        {state.k === 'done' && state.cached && (
          <span className="text-[12px] uppercase tracking-wider text-faint">cached</span>
        )}
      </div>
      ...
      {state.k === 'loading' && (
        <div className="sweep relative mt-3 overflow-hidden rounded-md border border-[color:var(--color-line)] px-3 py-6 text-center text-[14px] text-muted">
      ...
      {state.k === 'done' && (
        <div className="mt-4 space-y-4">
              ...
              <div className="space-y-2">
                {state.a.headline_translations.map((t, i) => (
                  <div key={i} className="rounded border border-[color:var(--color-line-soft)] p-2.5">
                    <div className="text-[12px] text-faint">{t.outlet}</div>
                    <div className="zh-text mt-0.5 text-[15px]">{t.original}</div>
                    <div className="mt-1 text-[15px] text-text">{t.english}</div>
                  </div>
                ))}
              </div>
            </Section>
          ...
          <Section title="How each bloc frames it">
            <div className="grid gap-2.5 sm:grid-cols-2">
              {state.a.framing_by_bloc.map((f, i) => (
                <div key={i} className="rounded border border-[color:var(--color-line-soft)] p-2.5">
                  <div className="text-[14px] font-medium text-[color:var(--color-accent)]">{f.bloc}</div>
                  <p className="mt-1 text-[14px] leading-snug text-muted">{f.frames_it_as}</p>
                  <p className="mt-1.5 text-[14px] leading-snug text-faint">
          ...
          <Section title="India relevance">
            <p className="text-[15px] leading-relaxed text-text">{state.a.india_relevance}</p>
          </Section>
          ...
          <p className="border-t border-[color:var(--color-line-soft)] pt-2.5 text-[13px] leading-relaxed text-faint">
