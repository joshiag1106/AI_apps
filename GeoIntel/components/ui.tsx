...
export function SectionTitle({ children, kicker, action, level = 2 }: { children: ReactNode; kicker?: string; action?: ReactNode; level?: 1 | 2 }) {
  ...
  return (
    <div className="flex items-end justify-between gap-4 mb-3">
      <div>
        {kicker && <div className="text-[12px] uppercase tracking-[0.18em] text-faint mb-1">{kicker}</div>}
        <Heading className="text-[17px] font-semibold tracking-tight text-text">{children}</Heading>
      </div>
      {action}
    </div>
  );
...
export function Badge({ children, tone = 'var(--color-muted)', title, solid = false }: { children: ReactNode; tone?: string; title?: string; solid?: boolean }) {
  return (
    <span
      title={title}
      className="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[12px] font-medium uppercase tracking-wider whitespace-nowrap"
      style={solid
...
export function Stat({ label, value, sub, tone }: { label: string; value: ReactNode; sub?: ReactNode; tone?: string }) {
  return (
    <div className="panel px-3.5 py-3">
      <div className="text-[12px] uppercase tracking-[0.16em] text-faint">{label}</div>
      <div className="mono-num text-2xl mt-1 leading-none" style={{ color: tone ?? 'var(--color-text)' }}>{value}</div>
      {sub && <div className="text-[13px] text-muted mt-1.5 leading-snug">{sub}</div>}
    </div>
  );
}
...
export function Trend({ value, suffix = '' }: { value: number; suffix?: string }) {
  ...
  return (
    <span className="mono-num text-[13px] inline-flex items-center gap-0.5" style={{ color: tone }}>
...
export function Empty({ children }: { children: ReactNode }) {
  return (
    <div className="panel px-4 py-8 text-center text-[15px] text-faint">
      {children}
    </div>
  );
}
